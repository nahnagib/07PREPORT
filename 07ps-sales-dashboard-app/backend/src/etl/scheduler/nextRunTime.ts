...
export function computeNextRunTime(cronExpression: string, timezone?: string): Date | null {
  try {
    const interval = parseExpression(cronExpression, timezone ? { tz: timezone } : undefined);
